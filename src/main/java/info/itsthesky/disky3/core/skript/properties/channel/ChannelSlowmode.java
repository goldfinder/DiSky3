...
public class ChannelSlowmode extends ChangeablePropertyExpression<GuildChannel, Number> {
    ...
    public void change(Event e, Object[] delta, Bot bot, Changer.ChangeMode mode) {
        ...
        channel.getManager().setSlowmode(value.intValue()).queue(null, ex -> DiSky.exception(ex, info));
    }

    @Override
    protected Number @NotNull [] get(@NotNull Event e, GuildChannel @NotNull [] source) {
        return new Number[] {((TextChannel) source[0]).getSlowmode()};
    }
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        return "slowmode of " + getExpr().toString(e, debug);
    }
