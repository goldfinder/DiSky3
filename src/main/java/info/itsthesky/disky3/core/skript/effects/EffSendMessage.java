...
public class EffSendMessage extends WaiterBotEffect<UpdatingMessage> {
    ...
    static {
        register(
                ...
                "send"+ (Configuration.PARSING_SEND_EFFECT.get() ? " discord" : "") +" [the] [message] %embedbuilder/string/messagebuilder% (in|to) [the] [channel] %channels/threads/users/members% [with [the] (row|component)[s] %-buttonrows/selectbuilders%] [and store (it|the message) in %-object%]"
        );
    }
