...
@Description("Represent the author icon of embed. You can't set the that without having an author text!")
@Since("3.0")
public class EmbedAuthorIcon extends SimplePropertyExpression<EmbedBuilder, String> {

    static {
        register(
                EmbedAuthorIcon.class,
                String.class,
                "[embed] author icon",
                "embedbuilder"
        );
    ...
    public @NotNull String convert(EmbedBuilder builder) {
        return builder.isEmpty() ? null : builder.build().getAuthor().getIconUrl();
    }
        ...
        if (mode == Changer.ChangeMode.SET && delta.length != 0 && delta[0] != null) {
            ...
            try {
                ...
                if (previousAuthorText == null)
                    throw new DiSkyException("You can't set an author icon icon without an author text!");

                embed.setAuthor(previousAuthorText, previousAuthorURL, value);

                if (useScope)
                    ScopeEmbed.lastEmbed.setAuthor(previousAuthorText, previousAuthorURL, value);
