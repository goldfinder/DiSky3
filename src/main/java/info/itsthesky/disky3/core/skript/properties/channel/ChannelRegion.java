...
public class ChannelRegion extends ChangeablePropertyExpression<GuildChannel, Region> {
    ...
    public void change(Event e, Object[] delta, Bot bot, Changer.ChangeMode mode) {
        ...
        final Region value = (Region) delta[0];
        if (value == null || channel == null) return;

        channel = bot.getCore().getGuildChannelById(channel.getId());

        channel.getManager().setRegion(value).queue(null, ex -> DiSky.exception(ex, info));
    }

    @Override
    protected Region @NotNull [] get(@NotNull Event e, GuildChannel @NotNull [] source) {
        return new Region[] {((VoiceChannel) source[0]).getRegion()};
    }
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        return "region of " + getExpr().toString(e, debug);
    }
