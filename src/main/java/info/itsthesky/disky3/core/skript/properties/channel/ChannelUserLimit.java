...
public class ChannelUserLimit extends ChangeablePropertyExpression<GuildChannel, Number> {

    static {
        register(
                ChannelUserLimit.class,
                Number.class,
                "[discord] (user[s] limit|max user[s])",
                "channel"
        );
    }
    ...
    public void change(Event e, Object[] delta, Bot bot, Changer.ChangeMode mode) {
        ...
        channel.getManager().setUserLimit(value.intValue()).queue(null, ex -> DiSky.exception(ex, info));
    }

    @Override
    protected Number @NotNull [] get(@NotNull Event e, GuildChannel @NotNull [] source) {
        return new Number[] {((VoiceChannel) source[0]).getUserLimit()};
    }
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        return "user limit of " + getExpr().toString(e, debug);
    }
