...
public class GuildRules extends ChangeablePropertyExpression<Guild, TextChannel> {

    static {
        register(
                GuildRules.class,
                TextChannel.class,
                "[discord] rule[s] [text( |-)]channel",
                "guild"
        );
    }
    ...
    public void change(Event e, Object[] delta, Bot bot, Changer.ChangeMode mode) {
        ...
        guild.getManager().setRulesChannel(value).queue(null, ex -> DiSky.exception(ex, info));
    }

    @Override
    protected TextChannel @NotNull [] get(@NotNull Event e, Guild @NotNull [] source) {
        return new TextChannel[] {source[0].getRulesChannel()};
    }
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        return "rule channel of " + getExpr().toString(e, debug);
    }
