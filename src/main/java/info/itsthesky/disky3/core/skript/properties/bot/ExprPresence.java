...
public class ExprPresence extends SimpleExpression<Activity> {
	...
	protected Activity @NotNull [] get(@NotNull Event e) {
		...
		switch (pattern) {
			...
			case 3:
				if (url == null)
					DiSky.exception(new DiSkyException("The streaming URL cannot be null or not set!"), node);
				if (!Activity.isValidStreamingUrl(url))
					DiSky.exception(new DiSkyException("The streaming URL specified for the presence is NOT valid! (Input: "+url+")"), node);
