 ...
 * @author ItsTheSky
 */
public final class BotGuildRegister extends RegisterDB {

    private static final BotGuildRegister instance;

    static {
        instance = new BotGuildRegister();
    }
    ...
    public void registerCommand(List<String> guilds, List<String> bots, SlashObject command) {
        command.setGlobalRegister(true);
        Bukkit.getScheduler().runTaskLater(
                DiSky.getInstance(),
                () -> registerCommand(command, SlashUtils.parseGuilds(guilds,
                        SlashUtils.parseBots(bots)
                )),
                10
        );
    }
