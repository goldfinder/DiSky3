...
public class GetNewsChannel extends SimpleExpression<NewsChannel> {
    ...
    static {
        Skript.registerExpression(
                ...
                "news( |-)channel (with|from) id %string% [(with|using) [bot] %-bot%]"
        );
    }
    ...
    public @NotNull String toString(@Nullable Event e, boolean debug) {
        return "news channel with id " + exprId.toString(e, debug);
    }

    @Override
    protected NewsChannel @NotNull [] get(@NotNull Event e) {
        String id = exprId.getSingle(e);
        Bot bot = exprBot.getSingle(e);
        if (id == null || bot == null) return new NewsChannel[0];

        return new NewsChannel[] {bot.getCore().getNewsChannelById(id)};
    }
